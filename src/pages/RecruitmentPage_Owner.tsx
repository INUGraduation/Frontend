...
function RecruitmentPage_Owner() {
  ...
  const fetchRecruitment = async () => {
    const url = `http://www.gaemoim.site/api/v1/recruitments/${recruitmentId}`;
  ...
  const fetchApplicants = async (applicationId) => {
    const url = `http://www.gaemoim.site/api/v1/applications/${applicationId}/submissions`;
    const token = localStorage.getItem('access_token');
    try {
      const response = await fetch(url, {
        method: 'GET',
  ...
  Gaemoim
      ...
      <div className="flex flex-col items-start mt-9 w-full text-2xl font-bold text-[#4A90E2] max-w-[1312px] max-md:max-w-full">
        ...
        <div className="flex gap-5 items-center mt-12 ml-20 whitespace-nowrap max-md:mt-10 max-md:ml-2.5">
          <img loading="lazy" src={recruitment.photo ? recruitment.photo : Profile} className="shrink-0 self-stretch aspect-square w-[50px]" alt="Profile" />
